...
def fechar_comanda(comanda_id: int, dados: FecharComanda):
    ...
    if comanda[1] == "fechada":
        cursor.close()
        conexao.close()
        return {"erro": "Esta comanda já está fechada."}
...
def listar_itens_comanda(comanda_id: int):
    ...
    cursor.execute(
        """
        SELECT itens_comanda.id, produtos.nome, itens_comanda.quantidade, itens_comanda.preco_unitario
        FROM itens_comanda
